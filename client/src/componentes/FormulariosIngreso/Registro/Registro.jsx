...
const Registro = ({setTipo})=>{
 ...
 const  emailRegex = /^[-\w.%+]{1,64}@(?:[A-Z0-9-]{1,63}\.){1,125}[A-Z]{2,63}$/i;
   ...
   if (!emailRegex.test(email)) {
    ...
    Axios.post('http://localhost:3001/usuario', {nombre, email, contrasena})
    .then(ele=>alert('registrado',ele))
    .catch(err=> {err.request.status && setEmailRepetido(true);return console.log(err)}); 
    e.target.reset(); 
    ...
    return(
        ...
        <div className="mensaje-bienvenida mb-5">
         ...
         <div className="grupo-formulario">
             ...
             <label className='etiqueta' id='email'>Email</label>
         ...
         <div className="grupo-formulario">
             <input type="password" name='contrasena2' onChange={handleChange} required/>
             <label className='etiqueta' >Repetir contraseña</label>
             <i className="fas fa-lock"></i>
         </div>
        
         {errorContraseña && <p className='error-contraseña'><i className="fas fa-unlock"></i> Las contraseñas no coinciden <i className="fas fa-unlock"></i></p>}
         {emailRepetido && <p className='error-contraseña'><i class="fas fa-exclamation-triangle"></i> El usuario ya se encuentra registrado <i className="fas fa-exclamation-triangle"></i></p>}
